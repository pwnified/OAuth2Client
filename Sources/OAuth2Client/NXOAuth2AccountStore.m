...
+ (NSDictionary *)accountsFromDefaultKeychain;
{
    ...
    if (status != noErr) {
        NSAssert1(status == errSecItemNotFound, @"Unexpected error while fetching accounts from keychain: %ld", (long)status);
        return nil;
    }
...
+ (void)storeAccountsInDefaultKeychain:(NSDictionary *)accounts;
{
    ...
    NSAssert1(err == noErr, @"Error while adding token to keychain: %ld", (long)err);
}
...
+ (void)removeFromDefaultKeychain;
{
    ...
    NSAssert1((err == noErr || err == errSecItemNotFound), @"Error while deleting token from keychain: %ld", (long)err);
